...
begin
  ...
  begin
    {$IFDEF UNICODE}
    if not CharInSet(wrkstr[i], ['0'..'9']) then
    {$ELSE}
...
begin
  ...
  begin
    ...
    weid := Copy(weid, 1, Length(weid)-1) + Copy(alt_weid, Length(alt_weid), 1); // fix wildcard checksum if required (transfer checksum from alt_weid to weid)
    result := oid;
    exit;
  end;
...
var
  PaddedInput: string;
begin
  // Input auf 32 Zeichen mit '0' auffüllen
  PaddedInput := StringOfChar('0', 32 - Length(Input)) + Input;

  // UUID formatieren
  Result := LowerCase(Format('%s-%s-%s-%s-%s', [
    Copy(PaddedInput, 1, 8),
